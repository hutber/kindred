	...
	render(){
      ...
      <div className={main.middle}>
	      <div className={main.container}>
		      ...
		      <Logo />
		      <h1>Enter Pin</h1>
		      <p className={style.p}>{this.props.pin.pinSafetyFilledIn && !this.props.pin.pinsSafetyMatch ? 'Pin incorrect, please enter again' : 'Please enter your time to continue.'}</p>
		      <div className={style.pinContainer}>
			      {this.props.pin.pinSafety.map((item, index) => <div key={`pinBox${index}`} className={`${style.pinItem} ${item!==null ? style.full : ''}`}></div>)}
		      </div>
